...
with st.sidebar:
    st.image(
        "https://placehold.co/150x50/667eea/ffffff?text=LOGO",
...
# PAGE 2: Sales Analytics
    ...
    with tab1:
        col1, col2, col3 = st.columns(3)
        with col1:
            metric_type = st.selectbox("Metric", ["Sales", "Units", "Cost"])
        with col2:
        ...
        if group_by == "Date":
            grouped = df_filtered.groupby('Date')[metric_type].sum().reset_index()
        else:
            grouped = df_filtered.groupby(group_by)[metric_type].sum().reset_index()
            ...
            with st.container(border=True):
                ...
                with col3:
                    if insight['priority'] == 'High':
                        st.error(f"🔴 {insight['priority']}")
                    elif insight['priority'] == 'Medium':
                        st.warning(f"🟡 {insight['priority']}")
                    else:
                        st.info(f"🟢 {insight['priority']}")

# PAGE 6: System Settings
elif page == "⚙️ System Settings":
    st.title("⚙️ System Settings & Configuration")
    
    tab1, tab2, tab3 = st.tabs(["📊 Dashboard", "📝 Forms", "🎨 Visuals"])
    
    with tab1:
        st.write("📊 Dashboard Settings")
        
    with tab2:
        st.write("📝 Form Settings")
